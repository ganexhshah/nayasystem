...
import { useRefundReport } from "@/hooks/useApi"
...
export default function RefundReportPage() {
  ...
  const { data: rows = [], isLoading } = useRefundReport(params) as { data: RefundRow[]; isLoading: boolean }

  function handleExport() {
    downloadCsv(
      buildReportFilename("refunds", from, to),
      ["Order", "Customer", "Refund Amount", "Payment Method", "Reference", "Date & Time"],
      rows.map((row) => [
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-semibold">Refund Report</h1>
        <p className="text-sm text-muted-foreground mt-0.5">
          All processed refunds and their payment details
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/50">
              {["Order", "Customer", "Refund Amount", "Payment Method", "Reference", "Date & Time"].map((h) => (
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-16 text-sm">No refunds found for this period.</TableCell></TableRow>
            ) : rows.map((row) => (
